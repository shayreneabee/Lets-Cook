...
for (const culture of api.augustAroundWorldWeeks) {
  const audit = api.augustCultureContentAudit(culture);
  assert(audit.recipes.length >= 5, `${culture.title} must have a substantial canonical recipe collection`);
  assert(audit.drinks.length >= 1, `${culture.title} must have at least one connected traditional drink`);
  assert.deepStrictEqual([...audit.missingRecipeIds], [], `${culture.title} has missing or unpublished recipe IDs: ${audit.missingRecipeIds.join(", ")}`);
  assert.deepStrictEqual([...audit.missingDrinkIds], [], `${culture.title} has missing or unpublished drink IDs: ${audit.missingDrinkIds.join(", ")}`);
...
const worldTopology = JSON.parse(fs.readFileSync(path.join(root, "data", "world-countries-110m.json"), "utf8"));
const countryGeometry = api.decodeWorldCountryTopology(worldTopology);
assert(countryGeometry.length >= 170, "World Map must render a complete real-world country boundary dataset");
const requestedGeographicDestinations = {
  philippines: { countryId: "608", lat: [4, 22], lon: [115, 130] },
  nigeria: { countryId: "566", lat: [4, 14], lon: [2, 15] },
  ghana: { countryId: "288", lat: [4, 12], lon: [-4, 2] },
  greece: { countryId: "300", lat: [34, 43], lon: [19, 30] },
  "puerto-rico": { countryId: "630", lat: [17, 19], lon: [-68, -65] },
  peru: { countryId: "604", lat: [-19, 1], lon: [-82, -68] },
  korea: { countryId: "410", lat: [33, 39], lon: [124, 132] }
};
for (const [destinationId, expected] of Object.entries(requestedGeographicDestinations)) {
  const destination = api.worldGlobeDestinationById(destinationId);
  assert(destination, `${destinationId} must remain an available globe destination`);
  assert(destination.lat >= expected.lat[0] && destination.lat <= expected.lat[1], `${destinationId} latitude must match its real-world location`);
  assert(destination.lon >= expected.lon[0] && destination.lon <= expected.lon[1], `${destinationId} longitude must match its real-world location`);
  assert.strictEqual(api.worldDestinationCountryIds[destinationId], expected.countryId, `${destinationId} must map to its real ISO numeric country geometry`);
  assert(countryGeometry.some((country) => country.id === expected.countryId && country.polygons.length), `${destinationId} must have real projected country boundaries`);
}
assert(!source.includes("const worldGlobeContinents"), "World Map must not fall back to hand-drawn continent approximations");
